...
from utils_package.nms import nms
from utils_package.bbox_transform import bbox_overlaps

import pdb


class VideoPostProcessor(object):
    def __init__(self, pred_boxes, scores, classes, video_id=''):
        print("Starting post-processing on video id {}".format(video_id))
        self.video_id = video_id
        self.pred_boxes = pred_boxes
        self.scores = scores
        self.num_frame_pairs = pred_boxes.shape[0]
...
#	self.alpha_l = 3.0 
    ...
    def class_paths(self, path_score_thresh=0.0):
        ...
        self.path_starts = final_tubes['ts'][keep]
        self.path_ends = final_tubes['te'][keep]
    ...
    def extract_action(self, _p, _q, _D, action):
        ...
        input:pred_path, time, D, cls_ix
        output:Ts,Te,Scores, Label, DpPathScore
        '''
        ...
        if inds.numel()==0:
            ...
            # Added
            _D_l=torch.zeros(_D.size())
            step=torch.zeros(_D.size())
            _D_l[action-1,:-1]=_D[action-1,1:]
            step=(_D_l[action-1,:].cuda()-_D[action-1,:])
            bool_step=step>torch.FloatTensor([[0.]]).cuda()
            index=(bool_step!=0).nonzero()
            ...
            if index.byte().any():
                ...
                ts=max(ts,index[0]).unsqueeze(0)
                te=min(te,index[-1]).unsqueeze(0)
                ...
                #score is the average score over dt
...
#	'''
    ...
    def generate_paths(self):
        for cls_ix in range(1, self.num_classes): # skip background
            ...
            for pair_ix in range(self.num_frame_pairs):
                ...
                pick = torch.nonzero(scores_t0>0.).view(-1)
    ...
    #For each class, link the paths frame by frame
    def incremental_linking(self, frames_boxes, frames_scores, frames_all_scores):
	    ...
	    # boxes detected in frame t0
            ...
            if t0==0: # If on first frame pair, initialize with all detections
                ...
                #last_boxes_lps:[number_of_paths_for_last_frame,4]
                #last_boxes_lps:[number_of_paths_t0,4]
                    ...
                    if live_paths_last_found[lp] < self.jumpgap:
                        ...
                        if box_to_lp_score.sum()>0: # check if there's at least one box match
                            ...
                            live_paths_boxes[lp] = torch.cat([live_paths_boxes[lp], 
                                                              frames_boxes[t0][max_ind,:].view(1,-1)], dim=0)
                            ...
                            live_paths_all_scores[lp] = torch.cat([live_paths_all_scores[lp], 
                                                              frames_all_scores[t0][max_ind].view(1,-1)], dim=0)
                ...
                for lp in range(lp_count):
                    olp = path_inds[lp]
                   # print("live_paths_last_found[lp]",live_paths_last_found[lp])
                    if live_paths_last_found[lp] < self.jumpgap:
...
#        print("live_paths_boxes",live_paths_boxes)
...
#        print("path_order_score",path_order_score)
